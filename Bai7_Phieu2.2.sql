...
create function caua(@diachi nvarchar(30))
returns @table_a table(
	MaHang nchar(10),
	TenHang nvarchar(20),
	DiaChi nvarchar(30),
	SoDT nvarchar(20),
...
begin
	insert into @table_b
		select SanPham.MaSP, TenSP, HangSX.MaHangSX, TenHang, SoLuong, MauSac, GiaBan, DonViTinh, MoTa, DiaChi, SoDT, Email, NgayXuat
...
create function cauc(@tenhang nvarchar(20), @choice int)
returns @table_c table(
	MaSP nchar(10),
	TenSP nvarchar(20),
	TenHang nvarchar(20),
	SoLuong int,
...
begin
	if(@choice=0)
		insert into @table_c
			...
			where TenHang=@tenhang and MaSP in(
				select MaSP
...
select * from cauc('Vinfone', 0)
select * from cauc('Vinfone', 1)


-- d. Hãy xây dựng hàm Đưa ra danh sách các nhân viên đã nhập hàng vào ngày được đưa vào từ bàn phím.
create function caud(@ngay date)
returns @table_d table(
	MaNV nchar(10),
	TenNV nvarchar(20),
	GioiTinh nchar(10),
	DiaChi nvarchar(30),
...
select * from caue(1000000, 20000000, 'Samsung')
...
return 
(
	select MaSP, TenSP, HangSX.MaHangSX, TenHang, SoLuong, MauSac, GiaBan, DonViTinh, MoTa, DiaChi, SoDT, Email
	from SanPham inner join HangSX on SanPham.MaHangSX=HangSX.MaHangSX
)
select * from cauf()