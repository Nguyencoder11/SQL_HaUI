...
create view cau3 as
select Ton.TenVT, sum(SoLuongX) as 'So luong xuat'
from Xuat inner join Ton on Xuat.MaVT=Ton.MaVT
group by Ton.TenVT
select * from cau3
...
create view cau4 as
select Ton.TenVT, SUM(SoLuongN) as 'So luong nhap'
from Nhap inner join Ton on Nhap.MaVT=Ton.MaVT
group by Ton.TenVT
select * from cau4

-- Đưa ra tổng soluong còn trong kho biết còn = nhap – xuất + tồn theo từng nhóm vật tư
CREATE VIEW CAU5 AS
select ton.mavt,ton.tenvt,sum(soluongN)-sum(soluongX) + sum(soluongT) as 'Tong SL con lai'
from nhap inner join ton on nhap.mavt=ton.mavt
